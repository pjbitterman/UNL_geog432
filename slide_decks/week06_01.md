...
2. Download "unit06inclass.zip" from Canvas
...
6. Then write a script that simultaneously prints each *field name* and *field type*
...
with arcpy.da.SearchCursor(fc,("NAME10")) as cursor:
    for row in cursor:
        print (row[0])
```
- Must indent all code beneath the "with"
## I will us "with" in (most of) my examples
